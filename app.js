...
const STRINGS = {
  zh: {
    ...
    'tripModal.namePlaceholder': '例如：2026 日本雪季之旅',
  ...
  en: {
    ...
    'authGate.firstNameLabel': 'First Name',
    'authGate.firstNamePlaceholder': 'Wei',
    'authGate.lastNameLabel': 'Last Name',
    'authGate.lastNamePlaceholder': 'Tan',
    'authGate.nicknameLabel': 'Nickname',
    ...
    'tripModal.namePlaceholder': 'e.g. Japan Ski Trip 2026',
...
/**
 * 登入/注册成功后才会真正跑的 App 初始化流程（原本直接写在 DOMContentLoaded 里）
 */
...
/**
 * 金额四舍五入到 2 位小数，跟原本 Code.gs 的 roundAmount 是同一套逻辑
 * @param {number} amount
 * @return {number}
 */
 ...
 * 跟原本 Code.gs 的 settlementAlgorithm 逻辑一致
 */
...
function repaymentRowToOldShape_(row) {
  ...
  return {
    ...
    ToMember: idToName[row.to_member_id] || '',
...
/**
 * 撈取这趟旅程所有未删除的还款纪录（依日期由旧到新排序，跟原本後端行为一致）
 */
...
/* ------------------------------------------------------------
   ...
   抓回来，在前端即时加总成金库状态（跟原本 Code.gs 的 handleGetPool 算法一致）。
   ------------------------------------------------------------ */
...
async function loadTripData() {
  ...
  try {
    ...
    if (!isNetworkError(error)) {
      throw error; // 不是网路问题（例如权限被拒），照原本行为往外丢给呼叫端处理
    }
 ...
 * 跟原本後端 isMemberInUse 的检查范围一致：只看 Payer / Participants，不看还款纪录
...
/**
 * 渲染「还款纪录」列表；编辑/删除按钮只有「当事人」（item.CanManage，后端依 CreatedByUserId 判断）
 * 才看得到，不是自己建立的还款纪录，这里只能看不能动（删除后会重新计算结算总览）
 */
...
function handleLeaveTripClick() {
  ...
  openConfirmModal(
    ...
    async () => {
      ...
      // 之後其他人看到的消费/还款还是完整的，跟原本 Code.gs 的行为一致
      const { error } = await supabaseClient
        .from('members')
        .update({ user_id: null })
        .eq('trip_id', currentTripId)
 ...
 * 之后要换报表的品牌视觉，只需要改这一处
 */
const REPORT_LOGO_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 450 130">
  <defs>
    <linearGradient id="prPurpleBg" x1="0%" y1="0%" x2="100%" y2="100%">
      <stop offset="0%" stop-color="#1E1B4B"/>
      <stop offset="100%" stop-color="#0F172A"/>
    </linearGradient>
    <linearGradient id="prYellowBeak" x1="0%" y1="0%" x2="100%" y2="100%">
      <stop offset="0%" stop-color="#FBBF24"/>
      <stop offset="100%" stop-color="#F59E0B"/>
    </linearGradient>
    <linearGradient id="prElectricBar" x1="0%" y1="0%" x2="100%" y2="0%">
      <stop offset="0%" stop-color="#38BDF8"/>
      <stop offset="100%" stop-color="#818CF8"/>
    </linearGradient>
  </defs>
  <g transform="translate(15, 15)">
    <rect x="0" y="0" width="100" height="100" rx="24" fill="url(#prPurpleBg)"/>
    <g transform="rotate(-12, 50, 50)">
      <circle cx="50" cy="27" r="13" fill="#FFFFFF"/>
      <circle cx="54" cy="24" r="2.8" fill="#1E1B4B"/>
      <path d="M 61 25 C 71 25, 75 29, 72 34 C 69 37, 61 34, 61 30 Z" fill="url(#prYellowBeak)"/>
      <rect x="22" y="46" width="56" height="10" rx="5" fill="url(#prElectricBar)"/>
